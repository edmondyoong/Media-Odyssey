...
public class AuthExceptionHandler {
    ...
    public ResponseEntity<AuthApiResponse> handleBadCredentials() {
        return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                ...
                .body(AuthApiResponse.error("AUTH_INVALID_CREDENTIALS", "Invalid credentials. Please try again."));
    }
    ...
    public ResponseEntity<AuthApiResponse> handleDisabled() {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
                .contentType(MediaType.APPLICATION_JSON)
                .body(AuthApiResponse.error("AUTH_DISABLED", "User is disabled. Please complete verification."));
    }
